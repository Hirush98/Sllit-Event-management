...
const API_URL = "http://localhost:8004/api/leaderboard";
...
const leaderboardService = {
  ...
  getParticipantScore: async (eventId, userId) => {
    try {
      ...
      throw new Error(
        error.response?.data?.message || "Failed to fetch participant score",
      );
    }
  },
  ...
  updateParticipantScore: async (eventId, userId, scoreData) => {
    try {
      const response = await leaderboardApi.put(
        `/event/${eventId}/user/${userId}`,
        scoreData,
      );
      return response.data.data;
  ...
  getCollegeLeaderboard: async () => {
    try {
      ...
      throw new Error(
        error.response?.data?.message || "Failed to fetch college leaderboard",
      );
    }
  },
};
